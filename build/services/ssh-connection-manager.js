        ...
        await new Promise(async (resolve, reject) => {
            ...
            if (config.socksProxy) {
                try {
                    ...
                    return reject(new Error(`Failed to create SOCKS proxy connection for [${key}]: ${err.message}`));
                }
            }
            if (config.privateKey) {
                try {
                    ...
                    return reject(new Error(`Failed to read private key file for [${key}]: ${err.message}`));
                }
            }
                ...
                return reject(new Error(`No valid authentication method provided for [${key}] (password, private key, or agent)`));
            }
    ...
    async executeCommand(cmdString, name, options = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            client.exec(cmdString, (err, stream) => {
                ...
                stream.on("data", (chunk) => (data += chunk.toString())); // Collect stdout data
                stream.stderr.on("data", (chunk) => (errorData += chunk.toString()) // Collect stderr data
                );
